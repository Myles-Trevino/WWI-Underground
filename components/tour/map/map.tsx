...
export default observer(function Message(): JSX.Element {
	...
	// Render.
	if(!state.tour.panorama) return <></>;

	const regexResult = /\d+/.exec(state.tour.panorama);
	if(!regexResult) throw new Error('The map is incompatible with the loaded panorama.');
	const panoramaIndex = parseInt(regexResult[0])-1;
	...
	points.forEach((value, index) => {
		svgPoints.push(<circle key={index} className={Styles.point}
			onClick={(): void => { state.tour.setPanorama(`Panorama ${index+1}`); }}
			cx={value.x} cy={value.y} r={pointRadius}/>);
	});

	return (<>
		<svg className={classNames('tile', Styles.svg)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1335.11 926.33">
			...
			{/* FOV. */}
			{(panoramaIndex < fovOffsets.length && !state.tour.loading) && <path className={Styles.fov} d="M-77.22-77.22,0,0,77.22-77.22A109.36,109.36,0,0,0-77.22-77.22Z" transform={`translate(${selectedPoint.x} ${selectedPoint.y}),rotate(${Three.MathUtils.radToDeg(state.tour.rotation.x)+fovOffset})`}/>}
